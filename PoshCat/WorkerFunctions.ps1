...
Function Update-McAfee {
    param($computer)
    icm -ComputerName $computer -scriptblock {start-process "c:\program files (x86)\mcafee\virusscan enterprise\mcupdate.exe" -ArgumentList '/update'}
    sleep 10 
    icm -ComputerName $computer -scriptblock {Start-Process "c:\program files (x86)\mcafee\common framework\cmdagent.exe" -ArgumentList '/p'}
}
...
Function IAVA-2013A0225 {
    param($comp)
    Try {
        $root = "\\detrdetddf002\nae\NEC\TechShare\APPLICATIONS\FIXES_PATCHES_SERVICE_PACKS\IAVA_fixes"
        Copy-Item -Path "$root\2013-A-0225" -Destination "\\$comp\c$\NEC" -Recurse
        Set-Content "\\$comp\c$\NEC\installme.bat" -value 'c:\NEC\2007_2013_a_0225.exe  /quiet'
        ICM -ComputerName $comp -ScriptBlock {Start-Process 'c:\NEC\installme.bat' -Wait}
        #Sleep 5
        #Remove-Item -Path "\\$comp\c$\NEC" -Force
    } 
    Catch {
        $_.Exception.Message
    }
...
Function SimpleCopy {
    param ($comp)
    md "$comp\c$\NECTESTTESTTESTMOTHERFU"
    Copy-Item -Path "\\Detrdetddf002\nae\NEC\TechShare\INCOMING\Jack\test" -Destination "$comp\c$\NECTESTTESTTESTMOTHERFU" -Recurse
}
